...
for f in os.listdir('./training_mask'):
    dem=0
    for i in os.listdir('./training_mask/' + f + '/'):
        name_tuple = os.path.splitext(i)
        if (('.txt' in name_tuple[1]))&('name' not in name_tuple[0]):
            if(dem<=(len(os.listdir('./training_mask/' + f + '/'))-1)*0.7):
                y_train.append(f)
                y_trainh.append(dem1)
            else:
                y_test.append(f)
                y_testh.append(dem1)
            k= open('./training_mask/' + f + '/'+i, "r")
            line = k.read()
            line = line[:]
...
x_train=np.array(x)
x_test=np.array(x1)
output_dim=len(os.listdir('./training_mask'))
##############################################################
...
model_relu.add(Dense(num1, activation='relu', input_shape=(512,),
...
# model_relu.add(Dense(4096, activation='relu',
#     kernel_regularizer=kernel_regul,
#     kernel_initializer=he_normal(seed=seeds)))
# model_relu.add(BatchNormalization())
# model_relu.add(Dropout(0.5))
...
model_relu.save('modelmask.h5',history)
