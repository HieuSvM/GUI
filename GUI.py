...
frame1 = LabelFrame(root,background="cyan")
frame2 = Label(frame1,background="cyan")
check = True
def npf32u8(np_arr):
...
my_listbox = Listbox(root,height=3)
my_listbox.place(x = 70, y = 630)
my_listbox.insert(END, "Ảnh")
...
def openfile():
    ...
    if (my_listbox.get(ANCHOR)== "Ảnh"):
        ...
        file = 0
        vs1 = VideoStream(src=file).start()
        time.sleep(0)
        check = True
        checkcam(vs1)
...
def checkimg(img):
    ...
    try:
        label1 = cop
        if (img.shape[1] > 600)|(img.shape[0]>600):
...
def ex():
    sys.exit()
Button(root, text ="Open File",width=20,height=3, command = openfile).place(x=240,y=630)
Button(root, text ="Exit",width=20,height=3, command = ex).place(x=450,y=630)
